...
class Rexe:
    ...
    def remote_exec_cmd(self, cmd):
        ...
        """
        return self.remote_exec_cmd(cmd, self._random_node())

    @dispatch(str, str)
    def remote_exec_cmd(self, cmd, node):
        """
    ...
    @dispatch(str)
    def remote_exec_cmd_async(self, cmd: str) -> dict:
        """
        ...
        """
        return self.remote_exec_cmd_async(cmd, self._random_node())

    @dispatch(str, str)
    def remote_exec_cmd_async(self, cmd: str, node: str) -> dict:
        """
    ...
    @dispatch(str)
    def remote_exec_cmd_multinode(self, cmd):
        """
        Function to execute command in multiple nodes parallely
        when node list isn't given.
        """
        return self.remote_exec_cmd_multinode(cmd, list(self.node_dict.keys()))

    @dispatch(str, list)
    def remote_exec_cmd_multinode(self, cmd, node_list):
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_exec = {executor.submit(
                self.remote_exec_cmd, cmd, node): node for node in node_list}
